...
- **丝滑体验**：任何操作都不能有卡顿感，地图滑动、图层切换、面板弹出必须流畅，始终保持 60fps
...
1. **主线程只做 UI**：所有数据计算（Gaussian、解析 JSON、生成网格）必须在 `DispatchQueue.global` 或 `Task { }` 中执行，结果再 dispatch 到主线程更新 UI
2. **MapPolygon 数量控制**：同一时刻地图上 MapPolygon 数量不超过 200 个；超出时按视野裁剪
3. **避免 ForEach 重建**：用 `id:` 稳定标识符，避免 SwiftUI 不必要的视图重建
4. **图片 / 渲染缓存**：CGImage 热力图只在 region 变化超过 20% 时重绘，其余复用
5. **网络请求防抖**：地图移动时，Overpass 等网络请求加 0.5s debounce，避免拖拽时频繁触发
